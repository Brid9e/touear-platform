 ...
 * @author L.cm
 */
public class BerserkerRequestMappingHandlerMapping extends RequestMappingHandlerMapping {
	...
	private RequestMappingInfo getApiVersionMappingInfo(Method method, Class<?> handlerType) {
		...
		// 如果url版本不存在
		if (nonUrlVersion) {
			String vsersionMediaTypes = new BerserkerMediaType(apiVersion.value()).toString();
