...
public class PlaceholderUtil {
	 ...
	 * 根据索引来替换指定模板中的占位符值
	 * 
	 * @param value		内容【如：请先缴清 ${0} 年的 ${1}】
	 * @param params	参数【如：{"2018", "学费"}】
	 * @return			如：请先缴清 2018 年的 学费
	 * @author wangqq
	 * @date 2019年12月5日 上午10:00:40
	 */
	public String resolverByIndex(String value, String... params) {
		if (params != null && params.length != 0) {
			for (int i = 0; i < params.length; i++) {
				value = value.replace(this.placeholderPrefix + i + this.placeholderSuffix, params[i]);
			}
		}
		return value;
	}
