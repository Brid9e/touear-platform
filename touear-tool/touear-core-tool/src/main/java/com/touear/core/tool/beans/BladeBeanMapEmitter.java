...
class BladeBeanMapEmitter extends ClassEmitter {
	private static final Type BEAN_MAP = TypeUtils.parseType(AbstractBladeBeanMap.class.getName());
	...
	public BladeBeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		...
		if (require != 0) {
			for (Iterator it = allProps.keySet().iterator(); it.hasNext(); ) {
				...
				boolean flag = (((require & AbstractBladeBeanMap.REQUIRE_GETTER) != 0) && !getters.containsKey(name)) ||
					(((require & AbstractBladeBeanMap.REQUIRE_SETTER) != 0) && !setters.containsKey(name));
				if (flag) {
					it.remove();
					getters.remove(name);
					setters.remove(name);
