...
public class JsonUtil {
	 ...
	 * @throws Exception
	 */
	public static <T> T parseNew(String content, Class<T> valueType) throws Exception {
		return getInstance().readValue(content, valueType);
	}
	...
	public static <T> T parse(byte[] bytes, TypeReference<?> typeReference) {
		try {
			return getInstance().readValue(bytes, (TypeReference<T>) typeReference);
		} catch (IOException e) {
			throw Exceptions.unchecked(e);
		}
	}
	...
	private static class JacksonObjectMapper extends ObjectMapper {
		...
		public JacksonObjectMapper() {
			...
			//日期格式化
			super.registerModule(new TouearJavaTimeModule());
			super.findAndRegisterModules();
		}
